...
import random
from DBN import DBN
from load_dataset import MNIST, CIFAR10
...
if __name__ == '__main__':
	cifar10 = CIFAR10()
	train_x, train_y, test_x, test_y = cifar10.load_dataset()

	layers = [512, 128, 64, 10]

	dbn = DBN(train_x.shape[1], layers, savefile='cifar10_trained_dbn.pt')
	dbn.train_DBN(train_x)

	model = dbn.initialize_model()

	completed_model = torch.nn.Sequential(model, torch.nn.Softmax(dim=1))
	torch.save(completed_model, 'cifar10_trained_dbn_classifier.pt')
	...
	progress.to_csv('CIFAR10_DBN_without_pretraining_classifier.csv', index=False)

	print("With Pre-Training")
	model, progress = train(completed_model, train_x, train_y, train_x, train_y, test_x, test_y)
	progress = pd.DataFrame(np.array(progress))
	progress.columns = ['epochs', 'test loss', 'train loss', 'test acc', 'train acc']
	progress.to_csv('CIFAR10_DBN_with_pretraining_classifier.csv', index=False)
	...
	progress = pd.DataFrame(np.array(progress))
	progress.columns = ['epochs', 'test loss', 'train loss', 'test acc', 'train acc']
	progress.to_csv('CIFAR10_DBN_with_pretraining_and_input_binarization_classifier.csv', index=False)